...
public class Progression {
    ...
    public static String[][] getPlayProgression() {
        ...
        for (int i = 1; i < MAX_COUNT_QUESTIONS; i++) {
            int indexOfMissedNum = Engine.getRandomNumber(MAX_INDEX_MISSED_NUM);
            int currentNum = Engine.getRandomNumber(MAX_START_NUM);
            int stepOfProgression = Engine.getRandomNumber(MAX_PROGRESSION_STEP);
            int missedNum = 0;
